#!/bin/bash
# ================================================================
# Import Let's Encrypt Certificate cho Vault
# Chạy trên máy Vault Server sau khi lấy cert bằng Certbot DNS challenge
# ================================================================
...
echo "================================================="
echo "⚙️  Đang thiết lập SSL Let's Encrypt cho Vault"
echo "🌐 Domain: ${VAULT_HOST}"
...
if [ ! -d "${LE_DIR}" ]; then
    ...
    echo "   sudo certbot certonly --manual --preferred-challenges dns -d ${VAULT_HOST}"
    echo ""
    echo "⚠️  Lưu ý: Bạn cần cấu hình bản ghi TXT '_acme-challenge.${VAULT_HOST}' tại DNS server."
    exit 1
fi
...
# Sao chép cert (fullchain + privkey) - Vault cần cả 2 file này
...
echo ""
echo "📝 Tiếp theo: Chạy 'make up' để khởi động Vault!"
